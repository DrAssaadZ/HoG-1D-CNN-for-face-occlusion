...
def get_image_class(img_name):
    # getting the name part from the image name exp: s01_01.jpg --> s01
    file_split = img_name.split('_')
...
def prepare_dataset(dataset_path, nbr_fold):
    ...
    for train_index_list, test_index_list in cross_val.split(img_list, img_classes):
        ...
        fold_train_path = 'dataset2/fold ' + str(current_fold) + '/training/'
        os.mkdir(fold_train_path)
        fold_test_path = 'dataset2/fold ' + str(current_fold) + '/testing/'
...
# main ----------------------------------------------------------
orig_dataset_path = 'E:/Master/Datasets/faces dataset/Georgia Tech database_crop/cropped_faces/'

# prepare_dataset(dataset_path=orig_dataset_path, nbr_fold=5)

# train and test paths
train_path = 'dataset2/fold 4/training/'
test_path = 'dataset2/fold 4/testing/'
...
for i in orientations:
    for j in k:
        hog_recognition(train_dataset_path=train_path, test_dataset_path=test_path, knn=j, total_tr_imgs=600, 
                total_tst_imgs=150, train_img_nbr_class=12, tst_img_nbr_class=3, orien=i)